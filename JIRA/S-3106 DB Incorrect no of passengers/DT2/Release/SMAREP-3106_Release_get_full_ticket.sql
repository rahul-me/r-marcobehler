...
/****** Object:  StoredProcedure [dbo].[cp_Get_Full_Ticket_Retail]    Script Date: 2/21/2022 6:48:35 AM ******/
SET ANSI_NULLS ON
GO
...
-- ============================================================================================
-- Author		:  Maunish Soni
-- Modified date:  22-Nov-2021
-- Description	:  SMAREP-3045 : Adyen Payment Gateway : Refunds : Get Failure reason if refund/reversal failed
-- ============================================================================================
-- Author		:  Maunish Soni
-- Modified date:  29-Nov-2021
-- Description	:  SMAREP-3208 : Adyen Payment Gateway : Ticket Retrival : fitered to get only single record(top) for ticket history
-- ============================================================================================
-- Author		:  Chirag Patel
-- Modified date:  30-DEC-2021
-- Description	:  SMAREP-3302 : add order in LeadConsumer table for Customer Address display issue while amendment
-- ============================================================================================
-- Author		:  Chirag Patel
-- Modified date:  12-JAN-2022
-- Description	:  SMAREP-3280 : add expired record in history table 
-- ============================================================================================
-- Author		:  Rahul Chauhan
-- Modified date:  21-FEB-2022
-- Description	:  SMAREP-3106 : TC shows incorrect number of passengers on the ticket 
-- ============================================================================================
...
INNER JOIN dbo.tbl_sales AS B 
 ...
 INNER JOIN tbl_agent_users AU 
      ...
      SET    status = 
             --select 
             CASE 
               WHEN BS.TransType = '' AND BS.Transstatus = 'R' THEN 'REFUNDED' 
			   WHEN BS.TransType = 'I' AND BS.Transstatus = 'R' THEN 'PAYMENT REVERSED' 
			   WHEN BS.TransType = '' AND BS.Transstatus = 'H' THEN 'PENDING REFUND' 
			   WHEN BS.TransType = 'I' AND BS.Transstatus = 'H' THEN 'PENDING PAYMENT REVERSAL' 
			   WHEN BS.TransType = '' AND BS.Transstatus = 'U' THEN 'REFUND FAILED' 
			   WHEN BS.TransType = 'I' AND BS.Transstatus = 'H' THEN 'PAYMENT REVERSAL FAILED' 
      ...
      FROM   @Baskets 
      WHERE  status NOT IN ( 'CANCELLED', 'REFUNDED', 'PAYMENT REVERSED', 'PENDING PAYMENT REVERSAL', 'PENDING REFUND', 'REFUND FAILED', 'PAYMENT REVERSAL FAILED') 
	...
	BEGIN	
		IF (SELECT 	status from @Baskets WHERE saleid = @selected_sale_id  ) NOT IN ('CANCELLED', 'REFUNDED', 'PAYMENT REVERSED', 'PENDING PAYMENT REVERSAL', 'PENDING REFUND', 'REFUND FAILED', 'PAYMENT REVERSAL FAILED')
		BEGIN 
	...
	END
     ...
     ON SI.sales_item_id = SIT.sales_item_id 
      ...
      FROM   @baskets B 
             ...
             LEFT JOIN netplans.dbo.tb_tick_typ 
  ...
  FROM   @BasketFares BF 
	 ...
	 IF  Isnull(@selected_sale_id,0) = 0
	  ...
	  DECLARE @CancelledDt DATETIME
	  SELECT @CancelledDt=sale_date  FROM   @baskets WHERE  status = 'CANCELLED'
	
      IF( @Status IN ( 'BOOKED', 'AMENDED', 'REFUNDED', 'PAYMENT REVERSED' ) ) -- By DA:(SMAREP-1276 :Added Refunded Status)
        SELECT TOP 1 @Status = CASE 
                                 WHEN @JourneyType NOT IN ( 'Open return booked' ) 
                                      AND arrivaltime < Getdate() AND @CancelledDt IS NULL THEN 'EXPIRED' 
                                 WHEN @JourneyType IN ( 'Open return booked' ) 
                                      AND Datediff(day, arrivaltime, Getdate()) >= 90 AND @CancelledDt IS NULL THEN 'EXPIRED' 
                                 ELSE @Status 
                               END 
        FROM   @Legs 
      ...
      SELECT @Status = CASE 
                         WHEN @Status = 'EXPIRED' THEN 
                           CASE 
                             WHEN EXISTS(SELECT 1 FROM   @baskets WHERE  status = 'REFUNDED') THEN 'EXPIRED & REFUNDED'
							 WHEN EXISTS(SELECT 1 FROM   @baskets WHERE  status = 'PAYMENT REVERSED') THEN 'PAYMENT REVERSED'
                             ELSE 'EXPIRED' 
                           END 
	...
	END
     ...
     DECLARE @tempBaskets TABLE            --SMAREP-1424 - UP
	  ...
	  --SMAREP-3280
	  DECLARE @isTicketExpired BIT  
	  SELECT TOP 1 @isTicketExpired = CASE 
										WHEN @JourneyType NOT IN ( 'Open return booked' ) 
										     AND arrivaltime < Getdate() AND @CancelledDt IS NULL THEN 1 
										WHEN @JourneyType IN ( 'Open return booked' ) 
										     AND Datediff(day, arrivaltime, Getdate()) >= 90 AND @CancelledDt IS NULL THEN 1 
										ELSE 0 
									  END 
        FROM   @Legs 
        ORDER  BY legnumber DESC         
      ...
      IF( @Status = 'EXPIRED' OR @isTicketExpired=1 ) 
        BEGIN 
	...
	DECLARE @Refund_transaction_type_id INT; 
	DECLARE @Pending_Refund_transaction_type_id INT; 
	DECLARE @Failed_Refund_transaction_type_id INT; 
	
	SELECT @Refund_transaction_type_id = transaction_type_id FROM tbl_transaction_types WHERE SMART_transstatus_alias = 'R'
	SELECT @Pending_Refund_transaction_type_id = transaction_type_id FROM tbl_transaction_types WHERE SMART_transstatus_alias = 'H'
	SELECT @Failed_Refund_transaction_type_id = transaction_type_id FROM tbl_transaction_types WHERE SMART_transstatus_alias = 'U'
      ...
      SELECT B.[saleid]         AS [saleId], 
		...
		WHERE B.status NOT IN ('REFUNDED', 'PAYMENT REVERSED', 'PENDING REFUND', 'PENDING PAYMENT REVERSAL', 'REFUND FAILED', 'PAYMENT REVERSAL FAILED')

	UNION ALL

		SELECT
			B.[saleid]         AS [saleId], 
             SIT.date_created                   AS saleDate, 
             B.agent_user_id                         AS agentUserId, 
             B.user_id                               AS userId, 
             au.[name], 
             B.agent_id                              AS agentId, 
             ...
             (SELECT group_name 
              ...
              WHERE  h.agent_id = ag.agent_id 
                     AND group_name = 'Travel-Shop') AS agentGroup, 
             CASE  
               WHEN SIT.transaction_type_id = @Refund_transaction_type_id AND b.status = 'REFUNDED' THEN 'REFUNDED' 
			   WHEN SIT.transaction_type_id = @Refund_transaction_type_id AND b.status = 'PAYMENT REVERSED' THEN 'PAYMENT REVERSED' 
			   WHEN SIT.transaction_type_id = @Pending_Refund_transaction_type_id AND (b.status = 'REFUNDED' or b.status = 'PENDING REFUND') THEN 'PENDING REFUND' 
			   WHEN SIT.transaction_type_id = @Pending_Refund_transaction_type_id AND (b.status = 'PAYMENT REVERSED' or b.status = 'PENDING PAYMENT REVERSAL') THEN 'PENDING PAYMENT REVERSAL' 
			   WHEN SIT.transaction_type_id = @Failed_Refund_transaction_type_id AND (b.status = 'REFUNDED' or b.status = 'PENDING REFUND'or b.status = 'REFUND FAILED') THEN 'REFUND FAILED' 
			   WHEN SIT.transaction_type_id = @Failed_Refund_transaction_type_id AND (b.status = 'PAYMENT REVERSED' or b.status = 'PENDING PAYMENT REVERSAL' or b.status = 'PAYMENT REVERSAL FAILED') THEN 'PAYMENT REVERSAL FAILED' 
               ELSE b.status
             END
			 ,Case b.category                             -- SMAREP-1424 - UP
				When '' Then 
			 ...
			 End as category
		FROM   @tempBaskets AS B 
			 JOIN dbo.tbl_sales_items SI with (nolock) 
			   ON SI.sale_id = b.saleid AND SI.product_id  = 1 AND SI.sales_item_id = (select MIN(TSI.sales_item_id) from tbl_sales_items TSI where TSI.sale_id = b.saleid)
			 JOIN dbo.tbl_sales_item_transactions SIT with (nolock)
			   ON SI.sales_item_id = SIT.sales_item_id AND SIT.sales_item_transaction_id IN (select top 2 TSIT.sales_item_transaction_id from tbl_sales_item_transactions TSIT where TSIT.sales_item_id = SIT.sales_item_id AND TSIT.transaction_type_id 
			   IN (@Refund_transaction_type_id,@Pending_Refund_transaction_type_id,@Failed_Refund_transaction_type_id) order by TSIT.sales_item_transaction_id asc)
			 JOIN dbo.tbl_agent_users AU 
               ON b.agent_user_id = au.agent_user_id 
             JOIN dbo.tbl_agents Ag 
               ON AU.agent_id = Ag.agent_id
		WHERE B.status IN ('REFUNDED', 'PAYMENT REVERSED', 'PENDING REFUND', 'PENDING PAYMENT REVERSAL', 'REFUND FAILED', 'PAYMENT REVERSAL FAILED')
    ...
    ----          end 
      ...
      SELECT  DISTINCT Isnull(L.NAME, '') AS addressType, 
                      ...
                      CASE a.county_id 
                        WHEN 0 THEN '' 
                        ELSE county 
                      END                AS county ,C.saleid --SMAREP-3302
      FROM   @LeadConsumer AS C 
		 ...
		 Order BY C.saleid DESC; --SMAREP-3302
...
sale_id                INT NOT NULL, 
 ...
 ON si.sales_item_id = sjl.sales_item_id 
     ...
     ON SOR.override_code = SO.override_reason 
	  ...
	  SELECT PPF.id	paymentFailureId,
			 PPF.sale_id saleId,
			 PPF.error_code, 
			 PPF.error_message
	  FROM [dbo].[tbl_Payment_Provider_Failures] PPF
	  WHERE ticket_number = @ticket_serial
  END 
