...
/****** Object:  StoredProcedure [dbo].[cp_RetailAPI_Basket_other_product_Purchase]    Script Date: 4/1/2021 5:09:08 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE procedure [dbo].[cp_RetailAPI_Basket_other_product_Purchase]
		...
		while @ID <= @AddonCount -- less than or equal to 
			begin
				...
				SET @offsetDays = 0
				 ...
				 BEGIN
					EXECUTE @RC = [dbo].[cp_get_product_duration_offset_by_product_sku_and_agent_code] 							  
							   @product_sku
							  ,@agent
							  ,@offsetDays OUTPUT
							...
							BEGIN
								...
								set To_date = DATEADD(DAY,@offsetDays, From_date)
								where
									ID= @ID
							END
				 END
