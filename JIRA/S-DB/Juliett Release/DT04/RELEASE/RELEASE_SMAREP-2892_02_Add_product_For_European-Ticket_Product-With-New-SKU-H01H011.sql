...
/*Add record in product for product 'European Ticket' with new SKU to have consitency of unique SKU for each products that serves through product catalogue */

EXEC @RC = [dbo].[cp_Add_Product] 
   @SMART_product_type_code = 'H01'
  ,@SMART_product_code = 'H011'
  ,@product_type_id = 5
  ,@product_family_id = 12
  ,@product_brand_id = 2
  ,@product_description = 'Eurolines Ticket'
  ,@valid_from = ''
  ,@valid_to = ''
  ,@print_template_id = 8
  ,@serial_required = 1
