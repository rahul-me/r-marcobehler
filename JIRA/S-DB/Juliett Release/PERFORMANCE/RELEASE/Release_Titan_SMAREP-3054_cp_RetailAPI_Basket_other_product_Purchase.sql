USE [Titan]
GO
/****** Object:  StoredProcedure [dbo].[cp_RetailAPI_Basket_other_product_Purchase]    Script Date: 04/10/2021 13:10:14 ******/
SET ANSI_NULLS ON
GO
...
-- Description	:	Added Insertion queries to record lead passenger role in consumer table. Added insertion queries to add email address and phone number for lead passenger (used in TravelCat)
-- ==================================================================================================================
-- Author		:	Maunish Soni
-- Create date	:	08-June-2021
-- Description	:	Added conditions while inserting records to tbl_Email_Addresses to make email address field non-mendatory for SMAREP-2706 (used in TravelCat)
-- ==================================================================================================================
-- Author		:	Rahul Chauhan
-- Create date	:	28 July 2021
-- Description	:	SMAREP-2785: Added insertions for country code
-- ==================================================================================================================
-- Author		:	Keval Panchal
-- Create date	:	07-October-2021
-- Description	:	SAMREP-3054 - Allow no distribution type for specific products SKU (H01H192, H01H191, H01H190, H60H001) 
-- ==================================================================================================================
...
--extract Distribution Details for getting email address which will be the email address for lead passenger as well
...
-- Add Lead Passenger as consumer
...
IF @@error <> 0
  BEGIN
      ...
      EXEC Sp_xml_removedocument
        @docHandle
      SET @Message = 'Failed to add lead cosumer details'
      RAISERROR('Failed to add lead cosumer details',11,1)
      GOTO errordetetcted
  END 

declare @ProductLeadConsumerID int
  
set @ProductLeadConsumerID = SCOPE_IDENTITY() 
print 'ProductLeadConsumer ID' 
print @ProductLeadConsumerID

	-- start adding email address for lead passenger
	if IsNull(@EmailAddress,'') <> ''
			Begin
				INSERT INTO [dbo].[tbl_Email_Addresses]
				 ...
				 VALUES(
					   @ProductLeadConsumerID
					   ,@EmailAddress
					   ,1
					   ,@LeadPaxEmailConsent)
			
			End
    -- end of adding email address for lead passenger
	
	-- start of adding phone number for lead consumer
	...
	-- end of adding phone number for lead consumer
-- end Lead Passenger as consumer
...
DECLARE @BillingAddress1 varchar(40)
			...
			-- start adding email address for paying consumer
			if IsNull(@EmailAddress,'') <> ''
				Begin
					INSERT INTO [dbo].[tbl_Email_Addresses]
						([consumer_id]
					...
					VALUES(
						...
						,@LeadPaxEmailConsent)
				End	
			-- end of adding email address for  paying consumer
...
------ (5) Not Applicable distribution type
	if @TicketDistributionType = 'Not Applicable'
		BEGIN
