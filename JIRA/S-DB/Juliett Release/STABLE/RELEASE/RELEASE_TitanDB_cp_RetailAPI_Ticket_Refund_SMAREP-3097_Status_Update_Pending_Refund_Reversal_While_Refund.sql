...
BEGIN
 ...
 --    		SET @Message = 'This Ticket is already refunded'
	...
	DECLARE @Refund_method varchar(80)
		SELECT
		...
		with (
				...
				refundMethod varchar(80) '@refundMethod'
			  )	 
...
,@transaction_id,@Is_RsoMode,@sale_id
	  ...
	  END
				...
				SET @Refund_method = Isnull(@Refund_method, '')
			   IF @Refund_method = ''
				BEGIN
					SET @Transaction_Type = ''
				END
			   IF @Refund_method = 'adyen-reversal'
				BEGIN
					SET @Transaction_Type = 'H'
				END
			   IF @Refund_method = 'adyen-refund'
				BEGIN
					SET @Transaction_Type = 'J'
				END				
			   ...
			   SELECT @sale_id,@Ticket,DATEDIFF(SECOND, '1970-01-01 00:00:00', GETDATE()),@agent_code,@user,'R',@Transaction_Type,@previous_sale_id,Highest_Fare_Paid_To_Date,Highest_Fare_Paid_To_Date_Outward,Highest_Fare_Paid_To_Date_Return
